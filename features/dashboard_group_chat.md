...
* **Strict Container Object Guard (`isSpaceObject`):** In both memory cache hit and database restoration branches of `handleFileClick`, `isSpaceObject` strictly checks container flags (`type === 'space'`, `type === 'workspace'`, `isProject`, `chats`) and excludes file objects with file extensions (`!file.name?.includes('.')`). Clicking **Edit** or the card header on a dashboard preview card resolves `specificFileMatch` directly to the custom tool file (`index.html`) or artifact document without misclassifying it as a space container.
