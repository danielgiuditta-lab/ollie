...
### Invariant 43: Parent Space Group Chat & Child Chat Session Exclusion (`processChatSession`)
- **Child Chats Exclusion Guard (`chatIdVal !== spaceId`)**: When constructing `space.chats` for each Space in `LeftNav.tsx`, `processChatSession` MUST enforce `chatIdVal !== spaceId` in addition to checking `c.messages && c.messages.length > 0`. Because root space sessions (`chatIdVal === spaceId`) contain welcome notices or group chat history, omitting `chatIdVal !== spaceId` pushed the parent space session into its own `space.chats` list, displaying a duplicate child row named after the space. Clicking this duplicate child row invoked `openChat(spaceId)` with `skipSelect = false`, causing `handleFileClick` to treat the click as a child artifact selection and anchor to a random library file. Restricting `space.chats` to `chatIdVal !== spaceId` guarantees that the primary parent space row is the sole group chat trigger for the space dashboard, and only true child authoring sessions (`chatIdVal !== spaceId`) reside below it.

### Invariant 44: One-Off / Child Chat Deletion & Active Context Routing (`handleRemoveTask`)
- **ContextMenu Trigger on Child Chats**: In `LeftNav.tsx`, right-clicking any child authoring chat under a Space or Home attaches `onContextMenu` and triggers the context menu popup (`setContextMenu`) displaying the "Remove" option.
