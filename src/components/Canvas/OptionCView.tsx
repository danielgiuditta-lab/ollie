              ...
              <div className="w-full max-w-[560px] relative z-30 flex flex-col items-center">
                ...
                <AnimatePresence>
                  {messages.some(m => (overlayNow - (m.createdAt || m._seenAt || Date.now())) < 30000) && (
                    <motion.div 
                      key="optionc-chat-blur"
                      initial={{ opacity: 0, y: 20, scaleY: 0.9 }}
                      animate={{ opacity: 1, y: 0, scaleY: 1 }}
                      exit={{ opacity: 0, y: 12, scaleY: 0.95 }}
                      transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }}
                      className="fixed inset-x-0 bottom-0 h-[38vh] max-h-[340px] z-20 pointer-events-none overflow-hidden origin-bottom"
                      style={{
                    ...
                    >
                      ...
                      <div 
                        className="absolute inset-0 pointer-events-none"
                        style={{
                          background: 'linear-gradient(to top, rgba(255, 255, 255, 0.40) 0%, rgba(255, 255, 255, 0.18) 45%, rgba(255, 255, 255, 0.04) 75%, rgba(255, 255, 255, 0.0) 100%)'
                        }}
                      />
                    </motion.div>
                  )}
                ...
                <AnimatePresence>
                  {messages.some(m => (overlayNow - (m.createdAt || m._seenAt || Date.now())) < 30000) && (
                    ...
                    >
                        ...
                        if (msg.role === 'user') {
                          return (
                            <div key={`user-${index}`} className={`w-full flex justify-end px-0 ${fadeClass}`}>
                              <UserMessage text={msg.text} theme="light" shadowClass="shadow-card" borderClass="border border-[#E9EEF6] dark:border-[#2B2D31]" />
                            </div>
                          );
                        }
                      ...
                      {isLoading && (
                        <div className="bg-white/95 text-slate-800 border border-[#E9EEF6] rounded-[24px] px-4 py-3 text-xs sm:text-sm font-normal max-w-[90%] shadow-card backdrop-blur-md flex items-center gap-3">
                          <OllieMascot variant="flat" size={24} state="idle" />
                          <span className="text-slate-600 font-medium font-sans">Ollie is thinking...</span>
                        </div>
                      )}
                    </motion.div>
                  )}
